...
volatile SD_CardInfo SDCardInfo;									//SD卡信息

//SD_ReadDisk/SD_WriteDisk函数专用buf,当这两个函数的数据缓存区地址不是4字节对齐的时候,
//需要用到该数组,确保数据缓存区地址是4字节对齐的.
volatile __align(4) u8 SDIO_DATA_BUFFER[512];						  
...
SD_Error SD_Init(void)
{
	...
	if(errorstatus==SD_OK)
	{
		errorstatus=SD_GetCardInfo((SD_CardInfo *)&SDCardInfo);	//获取卡信息
...
u8 SD_WriteDisk(u8*buf,u32 sector,u8 cnt)
{
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
			memcpy((u8 *)SDIO_DATA_BUFFER,buf,512);
		 	sta=SD_WriteBlock((u8 *)SDIO_DATA_BUFFER,lsector+512*n,512);//单个sector的写操作
			buf+=512;
		} 
	}else
	{
